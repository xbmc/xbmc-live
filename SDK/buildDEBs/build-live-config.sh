...
if ! ls live-config_*.deb > /dev/null 2>&1 ; then
	echo "Retrieving live-config package..."

	repoURL="http://live.debian.net/archive/packages/live-config/"
	if [ -z "$SDK_USELATESTDEBIANLIVEFILES" ]; then
	    pkgVersion="3.0~a30-1"
	else
	    pkgVersion=$(curl -x "" -s -f $repoURL | grep -v orig | grep 3.0 | tail -1 | grep -o '"3.0~[^\/]*' | sed -e "s/\"//g")
	fi
	latestPackage1="live-config_${pkgVersion}_all.deb"
	latestPackage2="live-config-upstart_${pkgVersion}_all.deb"
